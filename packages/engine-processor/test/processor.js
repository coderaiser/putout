...
// remark-lint cannot generate the same result as it was before parse,// it cannot distinguish "fix" and "report" either.// So at first run with "--fix" on markdown file it will change it to simplified format.// even if nothing found 🤦🤷//// In this case we can benefit from:// - simplifying format of remark rules (which are split on fix/report with help of options(!));// - splitting one big rule on a couple small rules;//// Otherwise it will mutate the tree and report all found errors, so only one big rule with// "filter" + "fix" joined and one message is supported ☝️.//// Which is hard to maintain.// So we should choose between "robotic" format 🤖 and change file with no errors found but// easy to maintain plugins similar to ones 🐊**Putout** has.// Or hard to maintain one big blob of processing with one message but change only on --fix.
